	...
	control: {
		...
		'businessexpo-color': {
			...
			init: function ( control ) {
				...
				// Init the control.
				businessexpo.input.color.init( control );
			},
			...
			template: function ( control ) {
				control.container.html(
					businessexpo.input.color.getTemplate( {
		...
		color: {
			...
			getTemplate: function ( data ) {
				...
				html +=
					...
					'" value="' +
					data.value +
					'" class="businessexpo-color-control" data-id="' +
					data['data-id'] +
					'"/>';

				return (
					'<div class="businessexpo-input-container" data-id="' +
					data.id +
					'">' +
					html +
					'</div>'
			...
			init: function ( control ) {
				var picker = jQuery(
					'.businessexpo-color-control[data-id="' + control.id + '"]'
				    ),
				    clear;
				...
				setTimeout( function () {
					clear = jQuery(
						'.businessexpo-input-container[data-id="' +
						control.id +
						'"] .wp-picker-clear'
					);
					if ( clear.length ) {
...
(
	function () {
		 ...
		 */
		wp.customize.businessexpoDynamicControl = wp.customize.Control.extend( {
			initialize: function ( id, options ) {
				...
				if ( !args.params.type ) {
					args.params.type = 'businessexpo-generic';
				}
			...
			ready: function () {
				...
				control.deferred.embedded.done( function () {
					control.initbusinessexpoControl( control );
				} );
			},
			...
			embed: function () {
				...
				wp.customize.section( sectionId, function ( section ) {
					if (
						'businessexpo-expanded' === section.params.type ||
						section.expanded() ||
			 ...
			 * @returns {void}
			 */
			initbusinessexpoControl: function ( control ) {
				if ( 'undefined' !== typeof businessexpo.control[control.params.type] ) {
					businessexpo.control[control.params.type].init( control );
					return;
				}
			...
			businessexpoValidateCSSValue: function ( value ) {
				var validUnits = [
					    'rem',
					    'em',
					    'ex',
...
_.each( businessexpo.control, function ( obj, type ) {
	wp.customize.controlConstructor[
		type
		] = wp.customize.businessexpoDynamicControl.extend( {} );
} );
 ...
 * Control: Slider.
 */
wp.customize.controlConstructor['businessexpo-slider'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		var control      = this,
		    ...
		    slider       = control.container.find( '.businessexpo-slider' ),
		...
		// Init the control.
		if ( !_.isUndefined( window.businessexpoControlLoader ) && _.isFunction( businessexpoControlLoader ) ) {
 ...
 * Control: Text.
 */
wp.customize.controlConstructor['businessexpo-text'] = wp.customize.Control.extend( {
...
wp.customize.controlConstructor['businessexpo-sortable'] = wp.customize.Control.extend( {
	...
	initbusinessexpoControl: function () {
		...
		var control = this;
		control.container.find( '.businessexpo-controls-loading-spinner' ).hide();
 ...
 * Control: Radio image.
 */
wp.customize.controlConstructor['businessexpo-radio-image'] = wp.customize.businessexpoDynamicControl.extend( {} );

/**
 * Control: Toggle.
 */
wp.customize.controlConstructor['businessexpo-toggle'] = wp.customize.businessexpoDynamicControl.extend( {
 ...
 */
wp.customize.controlConstructor['businessexpo-radio-buttonset'] = wp.customize.Control.extend( {} );

/**
 * Control: Dimensions.
 */
;
/* global dimensionsbusinessexpoL10n */
/* global businessexpoControlLoader */
wp.customize.controlConstructor['businessexpo-dimensions'] = wp.customize.businessexpoDynamicControl.extend( {
	...
	updateDimensionsValue: function ( context, value ) {
		...
		control.container.on( 'change keyup paste', '.' + context + ' input', function () {
			...
			// Notifications.
			control.businessexpoNotifications();
	 ...
	 */
	businessexpoNotifications: function () {
			...
			setting.bind( function ( value ) {
				...
				_.each( ['top', 'right', 'bottom', 'left'], function ( direction ) {
					if ( !_.isUndefined( value[direction] ) ) {
						if ( false === control.businessexpoValidateCSSValue( value[direction] ) ) {
							subs[direction] = dimensionsbusinessexpoL10n[direction];
						} else {
				...
				if ( !_.isEmpty( subs ) ) {
					message = dimensionsbusinessexpoL10n['invalid-value'] + ' (' + _.values( subs ).toString() + ') ';
...
;
/* global businessexpoControlLoader */
wp.customize.controlConstructor['businessexpo-background'] = wp.customize.Control.extend( {
	...
	initbusinessexpoControl: function () {
		...
		var control = this,
		    value   = control.setting._value,
		    picker  = control.container.find( '.businessexpo-color-control' );
 ...
 * Control: Editor.
 */
wp.customize.controlConstructor['businessexpo-editor'] = wp.customize.businessexpoDynamicControl.extend( {
	...
	initbusinessexpoControl: function () {
		...
		var id = 'businessexpo-editor-' + control_id;
