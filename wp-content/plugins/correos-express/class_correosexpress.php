...
class CorreosExpress
{
    ...
    public function cex_option_page()
    {
        ...
        $menus[] = [
            ...
            'icon_url'          => plugin_dir_url(__FILE__).'views/img/logo_pos_thumb.png',
            'position'          => 15
        ];
   ...
   /* public function cex_include_custom_order_status_to_reports($statuses)
    ...
    public function cex_shop_order_manual()
    {
        ...
        <fieldset class="rounded-2 CEX-background-bluelight2 border-0 px-3">
        <legend
        ...
        class="CEX-btn btn-large CEX-button-red my-auto d-none"
    ...
    public function cex_retornar_intervalo(){
        ...
        if (! wp_verify_nonce($nonce, 'cex-nonce')) {
            die('Security cex_retornar_intervalo');
        }*/
    ...
    public function cex_retornar_boton_update()
    {
        ...
        $cabecera .='<div class="row-fluid CEX-background-red CEX-text-white p-1 d-flex align-items-center">
        ...
        foreach ($results as $result) {
            ...
            <a id="'.$result->id.'" tabindex="" class="CEX-btn CEX-button-red cex_borrar_codigo_cliente d-inline-block" onclick="borrarCodigoCliente(this.id)">
    ...
    public function cex_retornar_select_remitentes()
    {
        ...
        if (sizeof($results)==0) {
            ...
            foreach ($results as $result) {
                if ($result->id_sender==$remitenteDefecto['id_sender']) {
    ...
    public function cex_retornar_remitentes()
    {
        ...
        foreach ($registros as $registro) {
            ...
            <td>
            <a id='".$registro->id_sender."' tabindex='' class='CEX-btn CEX-button-red cex_borrar_remitente d-inline-block' onclick='borrarRemitente(this.id);'>
            <i class='fas fa-times'></i>
            </a>
            </td>        
    ...
    public function cex_validar_logo(){
        $tipo = $_FILES["MXPS_UPLOADFILE"]['type'];        
    ...
    public function cex_guardar_logo(){        

        $comprobante = move_uploaded_file($_FILES["MXPS_UPLOADFILE"]["tmp_name"], plugin_dir_path(__FILE__).'views/img/logo-custom.jpg');

        if($comprobante ){
            $this->cex_actualizar_customer_options_interno("MXPS_UPLOADFILE",'../wp-content/plugins/correos-express/views/img/logo-custom.jpg' );
        }
        return $comprobante;     
    }
    ...
    public function cex_guardar_imagen_logo(){
        $comprobante = false;        
        $logo    = $_POST['MXPS_UPLOADFILE'];                
        if(strcmp($logo, 'undefined') !== 0 ){             
            if($this->cex_validar_logo()){
                if ($_FILES['MXPS_UPLOADFILE']['size'] >= 400000) {
                    $mensaje= array(
                        'type'      => 'error',
                        'title'     => esc_html(__('Imagen logo demasiado grande', 'cex_pluggin')),
                    ...
                    $comprobante = $this->cex_guardar_logo();
                }
            }else{
                $mensaje= array(
                    'type'      => 'error',
            ...
            unset($_REQUEST['MXPS_UPLOADFILE']);
            unset($_POST['MXPS_UPLOADFILE']);
        }  
    ...
    public function cex_guardar_customer_options()
    {
        ...
        if(isset($_FILES['MXPS_UPLOADFILE'])){        
           $comprobante = $this->cex_guardar_imagen_logo();                         
       }
    ...
    if (array_key_exists('MXPS_CHECKUPLOADFILE', $_POST)) {

        if(strcmp($_POST['MXPS_CHECKUPLOADFILE'], 'true') ==0){
            $retorno['imagenLogo']  = '../wp-content/plugins/correos-express/views/img/logo-custom.jpg';               
        }
        else{
            $retorno['imagenLogo']  = '';                
        }
...
public function cex_retornar_mapeo_transportistas()
{
    ...
    foreach ($metodos as $metodo) {
        ...
        '.$this->cex_retornar_select_productosActivosZonas($metodo->instance_id).'
        </div>';
    }
    ...
    //solo se graban envios a granel
        ...
        //Peso
            ...
            $savedship = array(
                ...
                'codigo_cliente'        =>intval($datos['codigo_cliente']),
            ...
            switch ($tipo_peticion) {
                ...
                $sql=$wpdb->prepare("INSERT INTO $nombreTabla (date, numcollect, numship, collectfrom, postalcode, id_order, id_mode, id_sender, type, kg, package, payback_val, insured_value, id_bc, mode_ship_name, status, id_ship_expired, id_group, note_collect, note_deliver, iso_code, devolution, deliver_sat, mailLabel, desc_ref_1, desc_ref_2, from_hour, from_minute, to_hour, to_minute, sender_name, sender_contact, sender_address, sender_postcode, sender_city, sender_phone, sender_country, sender_email, receiver_name, receiver_contact, receiver_address, receiver_postcode, receiver_city, receiver_phone, receiver_phone2, receiver_email, receiver_country, codigo_cliente, oficina_entrega, created_at, updated_at, WS_ESTADO_TRACKING, deleted_at, modificacionAutomatica) VALUES (%s,%s,%s,%s,%s,%d,%d,%d,%s,%d,%d,%d,%d,%d,%s,%s,NULL,NULL,%s,%s,%s,%d,%d,%d,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'',NULL,%s)", $savedship);
                $wpdb->query($sql);
                break;
                case 'update':
                $savedship['updated_at'] = date("Y-m-d H:i:s");
